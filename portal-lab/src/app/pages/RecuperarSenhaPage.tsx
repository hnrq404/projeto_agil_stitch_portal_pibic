    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-canvas px-4">
        <div className="card flex w-full max-w-md flex-col items-center gap-3 p-8 text-center">
          <span aria-hidden="true" className="material-symbols-outlined text-[40px] text-teal">mark_email_read</span>
          <h1 className="font-display text-xl font-bold text-navy">Solicitação registrada</h1>
          <p className="text-sm text-muted">
            Se o endereço <strong>{email}</strong> estiver cadastrado, você receberá as instruções
            de redefinição. Na S1, a redefinição também pode ser solicitada à PRPq
            (<span className="font-mono text-xs">pibic.suporte@prpq.universidade.br</span>).
          </p>
          <a className="btn-primary mt-2 w-full" href="/login">Voltar ao login</a>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-canvas px-4">
      <div className="card flex w-full max-w-md flex-col gap-4 p-8">
        ...
        <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-1">
            <label className="label" htmlFor="recovery-email">E-mail Institucional</label>
            <input
              id="recovery-email"
              type="email"
              required
        ...
        <a className="text-center text-xs text-teal hover:underline" href="/login">
          Voltar ao login
        </a>
      </div>
    </div>
